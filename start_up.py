...
class StartUp:
    ...
    def get_employess(self):
        url = (
            self.driver.find_element_by_class_name("mt1")
            .find_element_by_tag_name("a")
            .get_attribute("href")
        )
        str_nb_employees = str(
            self.driver.find_element_by_class_name("mt1")
            .find_element_by_tag_name("a")
            .find_element_by_tag_name("span")
            .get_attribute("innerHTML")
        )
        nb_employees = int("".join(char for char in str_nb_employees if char.isdigit()))
        print(f"{self.full_name} has {nb_employees} employees")
        self.driver.get(url)
        time.sleep(2)
    ...
    def get_data(self):
        self.full_name = (
            self.driver.find_element_by_class_name("t-24")
            .find_element_by_tag_name("span")
            .get_attribute("innerHTML")
        )
        ...
        try:
            self.bio = self.driver.find_element_by_class_name(
        ...
        with open(
            f"html_files/startups/{self.full_name}.html", "w", encoding="utf-8"
        ) as f:
            f.write(code_sr)
        ...
        with open(
            f"json_files/startups/{self.full_name}.json", "w", encoding="utf-8"
        ) as f:
